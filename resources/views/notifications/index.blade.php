                ...
                <div class="app-content-inner space-y-4">
                    <div class="tbl-wrap">
                        @forelse ($notifications as $n)
                            ...
                            @endphp
                            <div class="nlist-item {{ $n->isUnread() ? 'unread' : '' }}" data-id="{{ $n->id }}">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center gap-2">
                                        ...
                                        @endif
                                        <p class="text-sm font-semibold" style="color:var(--ink-0);margin:0;">{{ $n->title }}</p>
                                    </div>
                                    @if ($n->message)
                                        <p class="text-xs mt-1" style="color:var(--ink-2);line-height:1.5;">{{ $n->message }}</p>
                                    @endif
                                    <div class="flex items-center gap-3 mt-2 text-mono" style="font-size:10.5px;color:var(--ink-4);">
                                        <span><i class="fa-regular fa-clock text-[9px]"></i> {{ $n->created_at->diffForHumans() }}</span>
                                        <span>·</span>
                                        <span>{{ $n->created_at->format('d M Y H:i') }}</span>
                                        @if ($n->link)
                                            <span>·</span>
                                            <a href="{{ $n->link }}" onclick="markNotifReadInline(event, {{ $n->id }}, '{{ $n->link }}')" style="color:var(--cyan);">Buka detail →</a>
                                        @endif
                                    </div>
                                </div>
                                <div class="nlist-actions">
                                    ...
                                    @endif
                                    @if ($n->user_id)
                                        <button onclick="deleteNotif({{ $n->id }})" class="btn-icon danger" title="Hapus">
                        ...
                        @if ($notifications->hasPages())
                            <div class="tbl-footer">
                                ...
                                <div class="pager">
                                    ...
                                    @else
                                        <a href="{{ $notifications->previousPageUrl() }}"><i class="fa-solid fa-chevron-left text-[9px]"></i></a>
                                    @endif
                                    <span class="active text-mono">{{ $notifications->currentPage() }}</span>
                                    @if ($notifications->hasMorePages())
                                        <a href="{{ $notifications->nextPageUrl() }}"><i class="fa-solid fa-chevron-right text-[9px]"></i></a>
                                    @else
