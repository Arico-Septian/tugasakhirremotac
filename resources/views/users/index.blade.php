                        ...
                        <div class="stat-card acc-cyan">
                            ...
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    ...
                                    <p class="stat-meta" style="color:var(--mint);">+{{ $newUsersThisWeek ?? 0 }} this week</p>
                                </div>
                    ...
                    <div class="tbl-wrap">
                        <div class="tbl-toolbar">
                            <form method="GET" action="/users" class="flex-1 min-w-[200px] max-w-md">
                                <label class="search-input">
                                    ...
                                    <input name="search" value="{{ request('search') }}" placeholder="Search by username…" autocomplete="off">
                                </label>
                            </form>
                            <div class="segmented">
                                <a class="seg {{ !request('role') ? 'active' : '' }}" href="/users">All</a>
                                <a class="seg {{ request('role') == 'admin' ? 'active' : '' }}" href="/users?role=admin">Admin</a>
                                <a class="seg {{ request('role') == 'operator' ? 'active' : '' }}" href="/users?role=operator">Operator</a>
                                <a class="seg {{ request('role') == 'user' ? 'active' : '' }}" href="/users?role=user">User</a>
                            </div>
                        ...
                        <div class="flex items-center justify-between px-4 py-2.5" style="background:rgba(255,255,255,0.02);border-bottom:1px solid var(--line-soft);">
                            <p class="text-xs" style="color:var(--ink-3);">
                                Showing <span class="text-mono" style="color:var(--ink-0);">{{ $users->count() }}</span>
                                of <span class="text-mono" style="color:var(--ink-0);">{{ $totalUsers }}</span> users
                            </p>
                            ...
                            @forelse ($users as $user)
                                ...
                                <div class="user-row">
                                    <div class="flex items-center gap-3 min-w-0">
                                        ...
                                        <div class="min-w-0">
                                            <p class="text-sm font-semibold" style="color:var(--ink-0);line-height:1.2;">{{ $user->name }}</p>
                                            <p class="text-xs mt-1 flex items-center gap-1.5" style="color:var(--ink-3);">
                                                <i class="fa-regular fa-circle text-[8px] {{ $isOnline ? 'text-mint' : '' }}" style="{{ $isOnline ? 'color:var(--mint);' : '' }}"></i>
                                                {{ $statusText }}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="user-actions-wrap flex items-center gap-2 flex-wrap justify-end">
                                        ...
                                        <span class="pill {{ $isActive ? 'pill-online' : 'pill-error' }}" style="padding:3px 9px;font-size:10px;">
                                            <span class="dot"></span><span>{{ $isActive ? 'Active' : 'Inactive' }}</span>
                                        </span>

                                        @if ($user->id !== Auth::user()->id)
                                            <form action="/users/status/{{ $user->id }}" method="POST" class="inline-flex"
                                                  onsubmit="return confirm('{{ $isActive ? 'Nonaktifkan' : 'Aktifkan' }} user ini?')">
                                                @csrf
                                                <button type="submit" class="btn-icon {{ $isActive ? 'warning' : '' }}" style="{{ $isActive ? '' : 'color:var(--mint);background:var(--mint-soft);border-color:var(--mint-soft-2);' }}"
                                                        title="{{ $isActive ? 'Deactivate user' : 'Activate user' }}">
                                                    <i class="fa-solid {{ $isActive ? 'fa-user-slash' : 'fa-user-check' }} text-[11px]"></i>
                                                </button>
                                            </form>
                                            <button onclick="editRole({{ $user->id }}, '{{ $user->role }}')" type="button"
