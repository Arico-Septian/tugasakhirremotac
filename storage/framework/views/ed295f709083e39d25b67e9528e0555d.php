...
<head>
    ...
    <style>
        ...
        .user-row:hover { background: var(--panel-1); }
        .user-row:last-child { border-bottom: none; }
        @media (max-width: 720px) {
            .user-row { grid-template-columns: 1fr; }
            .user-actions-wrap { justify-content: flex-start; }
        }
        ...
        .avatar-status.online { background: var(--mint); box-shadow: 0 0 0 4px rgba(110,231,183,.18); }
    </style>
</head>
<body>
            ...
            <div class="app-content">
                <div class="app-content-inner space-y-4">
                        ...
                        <div class="stat-card acc-cyan">
                            ...
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    ...
                                    <p class="stat-meta" style="color:var(--mint);">+<?php echo e($newUsersThisWeek ?? 0); ?> this week</p>
                                </div>
                    ...
                    <div class="tbl-wrap">
                        <div class="tbl-toolbar">
                            <form method="GET" action="/users" class="flex-1 min-w-[200px] max-w-md">
                                <label class="search-input">
                                    ...
                                    <input name="search" value="<?php echo e(request('search')); ?>" placeholder="Search by username…" autocomplete="off">
                                </label>
                            </form>
                            <div class="segmented">
                                <a class="seg <?php echo e(!request('role') ? 'active' : ''); ?>" href="/users">All</a>
                                <a class="seg <?php echo e(request('role') == 'admin' ? 'active' : ''); ?>" href="/users?role=admin">Admin</a>
                                <a class="seg <?php echo e(request('role') == 'operator' ? 'active' : ''); ?>" href="/users?role=operator">Operator</a>
                                <a class="seg <?php echo e(request('role') == 'user' ? 'active' : ''); ?>" href="/users?role=user">User</a>
                            </div>
                        ...
                        <div class="flex items-center justify-between px-4 py-2.5" style="background:rgba(255,255,255,0.02);border-bottom:1px solid var(--line-soft);">
                            <p class="text-xs" style="color:var(--ink-3);">
                                Showing <span class="text-mono" style="color:var(--ink-0);"><?php echo e($users->count()); ?></span>
                                of <span class="text-mono" style="color:var(--ink-0);"><?php echo e($totalUsers); ?></span> users
                            </p>
                        ...
                        <div id="user-list">
                            <?php $__empty_1 = true; $__currentLoopData = $users; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $user): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <?php
                                    ...
                                    $statusText = $isOnline
                                        ? 'Online'
                                        : ($user->last_activity ? \Carbon\Carbon::parse($user->last_activity)->diffForHumans() : 'Offline');
                                    $isActive = $user->is_active ?? true;
                                ?>
                                <div class="user-row">
                                    <div class="flex items-center gap-3 min-w-0">
                                        <div class="user-avatar-wrap">
                                            <div class="avatar avatar-lg"><?php echo e(strtoupper(substr($user->name, 0, 1))); ?></div>
                                            <span class="avatar-status <?php echo e($isOnline ? 'online' : ''); ?>"></span>
                                        </div>
                                        <div class="min-w-0">
                                            <p class="text-sm font-semibold" style="color:var(--ink-0);line-height:1.2;"><?php echo e($user->name); ?></p>
                                            <p class="text-xs mt-1 flex items-center gap-1.5" style="color:var(--ink-3);">
                                                <i class="fa-regular fa-circle text-[8px] <?php echo e($isOnline ? 'text-mint' : ''); ?>" style="<?php echo e($isOnline ? 'color:var(--mint);' : ''); ?>"></i>
                                    ...
                                    <div class="user-actions-wrap flex items-center gap-2 flex-wrap justify-end">
                                        ...
                                        <span class="pill <?php echo e($isActive ? 'pill-online' : 'pill-error'); ?>" style="padding:3px 9px;font-size:10px;">
                                            <span class="dot"></span><span><?php echo e($isActive ? 'Active' : 'Inactive'); ?></span>
                                        </span>

                                        <?php if($user->id !== Auth::user()->id): ?>
                                            <form action="/users/status/<?php echo e($user->id); ?>" method="POST" class="inline-flex"
                                                  onsubmit="return confirm('<?php echo e($isActive ? 'Nonaktifkan' : 'Aktifkan'); ?> user ini?')">
                                                <?php echo csrf_field(); ?>
                                                <button type="submit" class="btn-icon <?php echo e($isActive ? 'warning' : ''); ?>" style="<?php echo e($isActive ? '' : 'color:var(--mint);background:var(--mint-soft);border-color:var(--mint-soft-2);'); ?>"
                                                        title="<?php echo e($isActive ? 'Deactivate user' : 'Activate user'); ?>">
                                                    <i class="fa-solid <?php echo e($isActive ? 'fa-user-slash' : 'fa-user-check'); ?> text-[11px]"></i>
                                                </button>
                                            </form>
                                            <button onclick="editRole(<?php echo e($user->id); ?>, '<?php echo e($user->role); ?>')" type="button"
                                                    class="btn-icon lavender" title="Edit role">
...
function closeEditModal() { document.getElementById('editRoleModal')?.classList.remove('is-open'); }
function editRole(id, role) { openEditModal(id, role); }

document.addEventListener('keydown', e => { if (e.key === 'Escape') { closeModal(); closeEditModal(); } });
document.getElementById('modal')?.addEventListener('click', e => { if (e.target === e.currentTarget) closeModal(); });
document.getElementById('editRoleModal')?.addEventListener('click', e => { if (e.target === e.currentTarget) closeEditModal(); });
...
function deleteUser(id) {
    ...
    .then(() => { window.smToast('User deleted', 'success'); setTimeout(() => location.reload(), 800); })
...
window.addEventListener('beforeunload', () => { if (pingInterval) clearInterval(pingInterval); });
</script>
