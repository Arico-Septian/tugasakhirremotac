...
<head>
    ...
    <style>
        ...
        .nlist-item:hover { background: var(--panel-1); }
        .nlist-item.unread { background: rgba(77, 212, 255, 0.05); }
        ...
        .nlist-icon.error   { background: rgba(248,113,113,.16); color: var(--coral); }
        .nlist-icon.warning { background: rgba(251,191,36,.16); color: var(--amber); }
        .nlist-icon.success { background: rgba(110,231,183,.16); color: var(--mint); }
        .nlist-icon.info    { background: rgba(77,212,255,.16); color: var(--cyan); }
        .nlist-actions {
            display: flex; gap: 6px;
            flex-shrink: 0;
            opacity: 0; transition: var(--t-fast);
        }
        .nlist-item:hover .nlist-actions { opacity: 1; }
        @media (max-width: 720px) { .nlist-actions { opacity: 1; } }
    </style>
</head>
<body>
                ...
                <div class="app-content-inner space-y-4">
                    <div class="tbl-wrap">
                        <?php $__empty_1 = true; $__currentLoopData = $notifications; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $n): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            ...
                            ?>
                            <div class="nlist-item <?php echo e($n->isUnread() ? 'unread' : ''); ?>" data-id="<?php echo e($n->id); ?>">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <?php endif; ?>
                                        <p class="text-sm font-semibold" style="color:var(--ink-0);margin:0;"><?php echo e($n->title); ?></p>
                                    </div>
                                    <?php if($n->message): ?>
                                        <p class="text-xs mt-1" style="color:var(--ink-2);line-height:1.5;"><?php echo e($n->message); ?></p>
                                    <?php endif; ?>
                                    <div class="flex items-center gap-3 mt-2 text-mono" style="font-size:10.5px;color:var(--ink-4);">
                                        <span><i class="fa-regular fa-clock text-[9px]"></i> <?php echo e($n->created_at->diffForHumans()); ?></span>
                                        <span>·</span>
                                        <span><?php echo e($n->created_at->format('d M Y H:i')); ?></span>
                                        <?php if($n->link): ?>
                                            <span>·</span>
                                            <a href="<?php echo e($n->link); ?>" onclick="markNotifReadInline(event, <?php echo e($n->id); ?>, '<?php echo e($n->link); ?>')" style="color:var(--cyan);">Buka detail →</a>
                                        <?php endif; ?>
                                    </div>
                                </div>
                                <div class="nlist-actions">
                                    ...
                                    <?php endif; ?>
                                    <?php if($n->user_id): ?>
                                        <button onclick="deleteNotif(<?php echo e($n->id); ?>)" class="btn-icon danger" title="Hapus">
                        ...
                        <?php if($notifications->hasPages()): ?>
                            <div class="tbl-footer">
                                <p>Page <?php echo e($notifications->currentPage()); ?> of <?php echo e($notifications->lastPage()); ?></p>
                                <div class="pager">
                                    <?php if($notifications->onFirstPage()): ?>
                                        <span class="disabled"><i class="fa-solid fa-chevron-left text-[9px]"></i></span>
                                    <?php else: ?>
                                        <a href="<?php echo e($notifications->previousPageUrl()); ?>"><i class="fa-solid fa-chevron-left text-[9px]"></i></a>
                                    <?php endif; ?>
                                    <span class="active text-mono"><?php echo e($notifications->currentPage()); ?></span>
                                    <?php if($notifications->hasMorePages()): ?>
                                        <a href="<?php echo e($notifications->nextPageUrl()); ?>"><i class="fa-solid fa-chevron-right text-[9px]"></i></a>
                                    <?php else: ?>
