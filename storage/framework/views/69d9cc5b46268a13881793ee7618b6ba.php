...
<script>
const roomNames = <?php echo json_encode($rooms->pluck('name')->map(fn($n) => str_replace('server ', 'srv ', $n))) ?>;
const roomTemps = <?php echo json_encode($rooms->pluck('temperature')->map(fn($t) => is_null($t) ? null : (float)$t)->values(), 15, 512) ?>;
